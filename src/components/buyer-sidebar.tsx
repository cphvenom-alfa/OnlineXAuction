...
import { Gavel, LayoutDashboard, Heart, CreditCard, Bell, User, LogOut, ArrowLeft } from "lucide-react";
import { signOut } from "@/server/better-auth/client";
import { api } from "@/trpc/react";

const navItems = [
  { href: "/buyer/dashboard",      label: "Dashboard",   icon: LayoutDashboard },
  { href: "/buyer/watchlist",      label: "Watchlist",   icon: Heart           },
  { href: "/buyer/transactions",   label: "Transactions", icon: CreditCard      },
  ...
  return (
    <aside className="w-60 h-full bg-white border-r border-gray-100 flex flex-col shrink-0 shadow-sm">
      <div className="px-4 py-4 border-b border-gray-100">
        <Link href="/auctions" className="flex items-center gap-2 mb-1">
        ...
        <div className="flex items-center gap-2.5 mt-2">
          ...
          <span className="font-black text-gray-900 text-base" style={{ fontFamily: "'Syne', sans-serif" }}>BidHub</span>
        </div>
      </div>
        ...
        {navItems.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href}
              ...
              <Icon className={`w-4.5 h-4.5 shrink-0 ${active ? "text-purple-600" : "text-gray-400"}`} style={{ width: 18, height: 18 }} />
              <span className="flex-1">{item.label}</span>
              {item.href === "/buyer/notifications" && notifCount && notifCount > 0 && (
